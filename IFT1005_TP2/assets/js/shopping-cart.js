...
function handleShoppingCartPage(){
    if (localStorage.panier) showCartContentOnly();
...
function showCartContentOnly(){
    ...
    getDetailedCartItemsData(detailedCartItemsData);
    ...
    detailedCartItemsData.forEach((cartItem) => {
        ...
        cartItemsElement.append(
            `<tr class="cartRowStyle">
                <td class="cartFirstColumnStyle cartRowStyle"><i onclick="removeItem(${cartItem.id});" class="fa fa-circle-xmark fa-2x"></i></td>
                <td><a href="product.html">${cartItem.name}</a></td>
                <td>${cartItem.price}</td>
                <td style="padding-left:5px;">
                    <i id="decrementQuantityButton${cartItem.id}"onclick="changeItemQuantity(${cartItem.id} , -1);" style="padding:3px;"class="fa fa-circle-minus fa-1x"></i>
                    ${cartItem.quantity}
                    <i onclick="changeItemQuantity(${cartItem.id} , 1);" style="padding:3px;" class="fa fa-circle-plus fa-1x"></i></td>
                <td class="cartFourthColumnStyle">${parseFloat(cartItem.totalPrice).toFixed(2)}$</td>
            </tr>`
        );
    });
...
function changeItemQuantity(itemId , quantityChange){
    cart =  JSON.parse(localStorage.getItem('panier'));
...
function getDetailedCartItemsData(detailedCartItemData){
